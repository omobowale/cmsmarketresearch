<div class="{{$styleClass}}">
    ...
    <p class="font-medium text-green-700 hover:text-green-600 cursor-pointer hover:font-bold col-span-4">
        <a href="/services"><i class="las la-arrow-left font-bold mr-2 fa-lg"></i>Back</a>
    </p>
    @endif
     ...
     @if(!$viewing)
     <p class="font-medium text-green-700 hover:text-green-600 cursor-pointer hover:font-bold col-span-4">
         <a href="/services/{{$service->id}}"><i class="las la-eye font-bold mr-2 fa-lg"></i>View</a>
     </p>
     @endif
    ...
    <x-jet-confirmation-modal wire:model="modalFormVisible">
        <x-slot name="title">
            <h3 class="font-bold text-xl">{{ __('Delete Service') }}</h3>
        </x-slot>

        <x-slot name="content">
            {{ __('Are you sure you would like to delete this service?') }}
        </x-slot>
        ...
        <x-slot name="footer">
            ...
            <x-jet-danger-button class="ml-2" wire:click="deleteService" wire:loading.attr="disabled">
                {{ __('Delete') }}
            </x-jet-danger-button>
        </x-slot>
