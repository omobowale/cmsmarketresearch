                ...
                <div class="sm:w-full mx-auto">
                  <div class="flex items-center space-x-5">
                    ...
                    <div class="block pl-2 font-semibold text-xl text-gray-700">
                      <h2 class="leading-relaxed mx-auto">Edit a blog</h2>
                  ...
                  <div class="divide-y divide-gray-200">
                    <div class="py-8 text-base leading-6 space-y-4 text-gray-700 sm:text-lg sm:leading-7">
                        <div class="flex flex-col">
                            ...
                            <select name="category" id="category" class="px-4 py-2 border focus:ring-gray-500 focus:border-gray-900 w-full sm:text-sm border-gray-300 rounded-md focus:outline-none text-gray-600" value="{{ old('category') ?? $blog->blogcategory->blog_category ?? '' }}">
                              @foreach (App\Models\BlogCategory::all() as $category)
                                <option @if($category->id == $blog->blog_category_id) {{"selected"}} @else {{""}} @endif value="{{$category->id}}">{{$category->blog_category}}</option>
                              @endforeach
                            </select>
                            @error('title')
