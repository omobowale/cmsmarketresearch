<x-app-layout>
    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            <a href="/others">{{ __('Logo >> ') }} </a> <small class="text-blue-600">Edit Logo</small>
        </h2>
    </x-slot>
                ...
                <div class="sm:w-full mx-auto">
                  <div class="flex items-center space-x-5">
                    ...
                    <div class="block pl-2 font-semibold text-xl text-gray-700">
                      <h2 class="leading-relaxed mx-auto">Edit a logo</h2>
                  ...
                  <div class="divide-y divide-gray-200">
                    <div class="py-8 text-base leading-6 space-y-4 text-gray-700 sm:text-lg sm:leading-7">
                      <div class="flex flex-col">
                        <label for="name" class="leading-loose">Logo Name<span class="text-red-500"> *</span></label>
                        <input name="name" id="name" type="text" class="px-4 py-2 border focus:ring-gray-500 focus:border-gray-900 w-full sm:text-sm border-gray-300 rounded-md focus:outline-none text-gray-600" placeholder="Enter logo name" value="{{$logo->name ?? old('name') ?? ""}}">
                        @error('name')
                      ...
                      <div class="flex flex-col">
                        ...
                        <select name="current" id="current" type="text" class="px-4 py-2 border focus:ring-gray-500 focus:border-gray-900 w-full sm:text-sm border-gray-300 rounded-md focus:outline-none text-gray-600">
                            <option value="no" @if($logo->current == "no") {{"selected"}} @else {{""}} @endif>no</option>
                            <option value="yes" @if($logo->current == "yes") {{"selected"}} @else {{""}} @endif>yes</option>
                        </select>
                        @error('current')
                          <div class="text-sm text-red-500">{{ $message }}</div>
                        @enderror
